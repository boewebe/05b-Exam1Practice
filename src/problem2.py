...
def problem2b(rect, n, delta, win):
    ...
    rect.attach_to(win)
    for k in range(n):
        x1 = rect.get_lower_left_corner().x - (k * delta)
        y1 = rect.get_lower_left_corner().y + (k * delta)
        x2 = rect.get_upper_right_corner().x + (k * delta)
        y2 = rect.get_upper_right_corner().y  - (k * delta)
